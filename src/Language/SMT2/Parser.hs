...
removeComment = rc T.empty
  where
    ...
    rc acc rest =
      if T.null rest
        then acc
          ...
          let (c, cs) = (T.head rest, T.tail rest)
              f = case c of
                '"' -> capture "\"" (T.cons '"')
                '|' -> capture "|" (T.cons '|')
                ';' -> capture "\n\r" (const $ T.singleton ' ')
    ...
    capture stops after acc cs =
      let (captured, rest) = T.break (`elem` stops) cs
       in if T.null rest
            then acc <> after captured
            else
              let (s, ss) = (T.head rest, T.tail rest)
...
specConstant =
  SCDecimal <$> try decimal -- numeral can be a prefix
...
identifier =
  IdSymbol <$> symbol -- symbol cannot start with (, so no ambiguity
    <|> idIndexed
    <?> "identifier"
  where
...
attribute =
  AttrKeyValue <$> try (keyword <* spaces1) <*> attributeValue
    <|> AttrKey <$> keyword
    <?> "attribute"
...
term =
  TermSpecConstant <$> try specConstant
...
funSymbolDecl =
  try (betweenBrackets funConstant)
...
theoryAttribute =
  tryAttr "sorts" *> betweenBrackets (TASorts <$> sepSpace1 sortSymbolDecl)
...
logicAttribute =
  tryAttr "theories" *> betweenBrackets (LATheories <$> sepSpace1 symbol)
...
datatypeDec =
  DDNonparametric <$> betweenBrackets (sepSpace1 constructorDec)
    <|> parametric
...
resInfo =
  IRErrorBehaviour <$> (tryAttr "error-behavior" *> resErrorBehaviour)
